...
/**
 * How a setup should be read differs meaningfully by timeframe — a 1m
 * scalp and a 1d swing are different games. This drives both the prompt
 * language and the entry-deviation tolerance used in validation, so a
 * daily setup isn't wrongly rejected for having an entry 2% from price
 * (normal on that timeframe) while a 1m setup isn't allowed to drift
 * that far from current price (which would make it stale on arrival).
 */
...
function getTimeframeProfile(interval: string): TimeframeProfile {
  switch (interval) {
    ...
    case "5m":
      return {
        style: "scalp",
        entryTolerancePct: 0.3,
        description: "This is a scalping timeframe. Setups must be immediately actionable — entry must be at or extremely close to current price, since a scalp thesis goes stale within minutes. Favor tight stops referencing ATR over structural swing levels, which are too slow-moving to matter here.",
      };
    case "15m":
    case "30m":
    case "1h":
      return {
        style: "intraday",
        entryTolerancePct: 0.75,
        description: "This is an intraday timeframe. Balance structural swing levels with short-term momentum. Entry should reflect a realistic pullback/retest zone reachable within the current session, not a level price may not revisit for days.",
      };
    case "2h":
    case "4h":
      return {
        ...
        description: "This is a swing timeframe. Structural swing high/low levels and SMA position matter more than short-term noise. Entry can reflect a realistic multi-hour to multi-day pullback zone rather than requiring price to be there right now.",
      };
    case "1d":
    case "1w":
    case "1M":
      return {
        style: "position",
        entryTolerancePct: 3.0,
        description: "This is a position/swing-trading timeframe measured in days to weeks. Prioritize major structural levels, trend direction, and macro context over short-term price action. Entry may reflect a level price reaches over the coming days — do not require it to match the current close.",
      };
    default:
      return {
        style: "intraday",
        entryTolerancePct: 1.0,
        description: "Standard intraday evaluation.",
      };
  }
}
...
function validateTradePosition(
  ...
  const entryDeviationPct = Math.abs((entry - latestPrice) / latestPrice) * 100;
  if (entryDeviationPct > entryTolerancePct) {
    console.warn(`[AI Validation] Rejected trade: entry ${entry} deviates ${entryDeviationPct.toFixed(2)}% from live price ${latestPrice} (tolerance ${entryTolerancePct}%)`);
    return null;
  }
...
export class AiInsightService {
  public static async generateMarketInsight(marketData: MarketDataInput): Promise<MarketInsight> {
    ...
    try {
      ...
      const structureNote = recentSwingHigh != null && recentSwingLow != null
        ? `Local range (lookback scaled to this timeframe): support ${recentSwingLow}, resistance ${recentSwingHigh}. "Discount" = lower third of this range, "Premium" = upper third.`
        : `No local support/resistance range was provided — treat structural conviction as capped at MEDIUM, since discount/premium cannot be confirmed against real levels.`;
...
${profile.description}
Entry must be within ${profile.entryTolerancePct}% of current price (${latestPrice}) — this tolerance is calibrated to the ${interval} timeframe specifically; do not apply a different timeframe's standard.
...
STEP 3 — RISK & CONVICTION
- Entry: Must be within ${profile.entryTolerancePct}% of current price (${latestPrice}), per the timeframe context above.
- Stop-Loss: Maximum distance of ${riskPercent}% from entry${atr ? `, referencing ATR (${atr}) for placement` : ""}.
- Target: Stop-loss distance x ${riskRewardRatio}.
- Reduce confidence or return null if funding rate or 24h volume strongly opposes the trade direction.
- Reduce confidence or return null if ADX indicates a non-trending market.
...
`;
      ...
      console.log(`[AI Service] Sending request to Gemini for ${symbol} (${interval}, ${profile.style}), prompt length: ${prompt.length} chars`);
