...
export default function AiInsightsSidebar() {
  ...
  const [modalStep, setModalStep] =
    useState<"closed" | "choose_ai_or_custom" | "confirm_ai_target" | "success">("closed");
  ...
  useEffect(() => {
    ...
    const handleInsightError = (err: { symbol: string; message: string }) => {
      ...
      // ✅ FIX: Ignore "No structural change" errors to prevent red error spam
      if (err.message.includes("No structural change detected")) {
        console.log("[Sidebar] No structural change detected. Skipping UI error.");
        return; 
      }
    ...
    <>
              ...
              {modalStep === "choose_ai_or_custom" && (
                <div className="space-y-4">
                  <p className="text-zinc-300 text-xs leading-relaxed">
                    ...
                    <strong className="text-blue-400">${formatPrice(tradePosition.target)}</strong>?
                  </p>
                  <div className="grid grid-cols-2 gap-2 pt-2">
                    ...
                    <Button
                      ...
                      className="border-zinc-700 bg-zinc-900 text-zinc-300 hover:bg-zinc-800 text-xs font-bold"
                    >
                      No, Custom
                    </Button>
                  </div>
              ...
              {modalStep === "confirm_ai_target" && (
                <div className="space-y-4">
                  <p className="text-zinc-300 text-xs leading-relaxed">
                    ...
                    <strong className="text-zinc-100">{activeSymbol}</strong> at target price{" "}
                    <strong className="text-blue-400">${formatPrice(tradePosition.target)}</strong>?
                  </p>
                  <div className="grid grid-cols-2 gap-2 pt-2">
                    <Button
                    ...
                    <Button
                      ...
                      disabled={isSubmittingAlert}
                      className="border-zinc-700 bg-zinc-900 text-zinc-300 text-xs font-bold"
                    >
                      Back
                    </Button>
                  </div>
      ...
      <aside className="w-full h-full bg-black flex flex-col p-5 font-mono text-xs overflow-y-auto relative custom-scrollbar text-zinc-100">
        <div className="flex items-center justify-between pb-4 border-b border-zinc-800 mb-4 shrink-0">
          ...
          <Badge
            variant="outline"
            className="text-[10px] text-zinc-400 bg-zinc-900/80 border-zinc-800 px-2.5 py-1 rounded-md"
          >
        ...
        <div className="flex-1 flex flex-col space-y-4">
          {!isConnected ? (
            <Card className="p-4 bg-zinc-950/50 border-zinc-800 rounded-xl text-zinc-500 text-[11px]">
            ...
            <Card className="p-6 bg-zinc-950/50 border-zinc-800 rounded-xl space-y-3 mt-2">
              ...
              <p className="text-zinc-200 text-sm font-medium transition-all duration-300">
            ...
            <div className="space-y-4 pb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Card className="p-3 bg-zinc-950 border-zinc-800 rounded-xl flex items-center justify-between">
                    <span className="text-zinc-400 flex items-center gap-1.5 text-[11px]">
                      <Activity className="w-3.5 h-3.5 text-blue-400" /> RSI
                    </span>
                    <span className="font-bold text-zinc-200 font-mono text-sm">
                      {insight.indicators.rsi != null ? insight.indicators.rsi.toFixed(1) : "N/A"}
                    </span>
                  </Card>
                  <Card className="p-3 bg-zinc-950 border-zinc-800 rounded-xl flex items-center justify-between">
                    <span className="text-zinc-400 flex items-center gap-1.5 text-[11px]">
                      <TrendingUp className="w-3.5 h-3.5 text-emerald-400" /> SMA
                    </span>
                    <span className="font-bold text-zinc-200 font-mono text-sm">
                      {insight.indicators.sma != null ? formatPrice(insight.indicators.sma) : "N/A"}
                    </span>
                  </Card>
                </div>
              )}
              ...
              <Badge
                className={`w-full justify-between px-3 py-2 rounded-xl border text-[11px] font-bold ${CONFIDENCE_STYLES[confidence as Confidence]}`}
              >
                  ...
                  <Card className="p-3.5 bg-zinc-950 border-zinc-800 rounded-xl space-y-3">
                    <div className="flex items-center justify-between text-zinc-100 font-bold">
                      <span className="flex items-center gap-1.5 text-xs">
                        <Sliders className="w-3.5 h-3.5 text-blue-400" /> {tradePosition.side} setup
                      </span>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-2 pt-2 text-[11px]">
                      <div>
                        ...
                        <strong className="text-zinc-100">${formatPrice(tradePosition.entry)}</strong>
                      </div>
                      <div>
                        <span className="text-zinc-500">Stop:</span>{" "}
                        <strong className="text-rose-500">${formatPrice(tradePosition.stopLoss)}</strong>
                      </div>
                      <div>
                        <span className="text-zinc-500">Target:</span>{" "}
                        <strong className="text-emerald-500">${formatPrice(tradePosition.target)}</strong>
                      </div>
                    </div>
                  </Card>
                  ...
                  <Card className="p-3.5 bg-zinc-950 border-zinc-800 rounded-xl space-y-3">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        ...
                        className="w-full bg-zinc-900 border-zinc-800 rounded-lg p-1.5 h-8 text-zinc-200 font-mono text-xs placeholder:text-zinc-600"
                      />
                    </div>
                    {accountBalance > 0 && (
                      <>
                        ...
                        <div className="grid grid-cols-2 gap-2 pt-2 text-[11px]">
                          <div>
                            <span className="text-zinc-500">Risking:</span>{" "}
                            <strong className="text-rose-500">${formatPrice(riskAmountUSD)}</strong>
                            <span className="text-zinc-400"> ({riskPercent}%)</span>
                          </div>
                          <div>
                            <span className="text-zinc-500">Position size:</span>{" "}
                            <strong className="text-emerald-500">{positionSize.toFixed(4)} units</strong>
                          </div>
                        </div>
                        {profitBreakdown && (
                          <>
                            ...
                            <div className="grid grid-cols-2 gap-2 pt-2 text-[11px]">
                              <div>
                                <span className="text-zinc-500">If TP hits:</span>{" "}
                                <strong className="text-emerald-500">+${formatPrice(profitBreakdown.profitUSD)}</strong>
                              </div>
                              <div>
                                <span className="text-zinc-500">If SL hits:</span>{" "}
                                <strong className="text-rose-500">-${formatPrice(profitBreakdown.lossUSD)}</strong>
                              </div>
                            </div>
                          </>
                        )}
              ...
              {hasActionableTrade && tradePosition && (
                <div className="pt-2">
                  <Button
                    onClick={() => setModalStep("choose_ai_or_custom")}
                    className="w-full py-3 bg-blue-600 hover:bg-blue-500 text-white font-bold rounded-xl text-xs shadow-lg shadow-blue-500/20 h-10"
                  >
            ...
            <div className="text-center py-16 text-zinc-500 space-y-2">
              <p>
                ...
                <strong className="text-zinc-300">{activeSymbol}</strong>...
              </p>
            </div>
          )}
        </div>
